...
public class EvenementSponsorController {
    ...
    public void initialize() {
        ...
        btnGeneratePdf.setOnAction(event -> {
            EvenementSponsor selectedSponsor = lv_evenementSponsor.getSelectionModel().getSelectedItem();
            if (selectedSponsor != null) {
                try {
                    generatePdf(selectedSponsor); // Générer le PDF pour l'élément sélectionné
    ...
    private void generatePdf(EvenementSponsor evenementSponsor) throws SQLException, IOException, DocumentException {
        ...
        document.add(new Paragraph("Fait à " + evenementRecherche.getLieuEvent() + ", le " + evenementSponsor.getDatedebutContrat(), blackFont));
...
public class EvenementSponsorController {
    ...
    public void initialize() {
        ...
        // Configurer la CellFactory pour afficher un bouton "Générer PDF" pour chaque élément
        ...
        lv_evenementSponsor.setCellFactory(new Callback<ListView<EvenementSponsor>, ListCell<EvenementSponsor>>() {
            ...
            public ListCell<EvenementSponsor> call(ListView<EvenementSponsor> param) {
                return new ListCell<EvenementSponsor>() {
                    ...
                    {
                        // Configurer le HBox
                        hbox.setSpacing(10); // Espace entre le texte et le bouton
                        HBox.setHgrow(hbox, Priority.ALWAYS); // Permet au texte de prendre tout l'espace disponible
                        ...
                        btnGeneratePdf.setOnAction(event -> {
                            ...
                            if (evenementSponsor != null) {
                                try {
                                    generatePdf(evenementSponsor); // Générer le PDF pour l'élément sélectionné
                                } catch (SQLException | IOException | DocumentException e) {
                                    e.printStackTrace();
                                    System.out.println("Erreur lors de la génération du PDF: " + e.getMessage());
                                }
                    ...
                    protected void updateItem(EvenementSponsor evenementSponsor, boolean empty) {
                        ...
                        if (empty || evenementSponsor == null) {
                            ...
                            // Afficher le texte de l'association et le bouton dans un HBox
                            hbox.getChildren().clear(); // Nettoyer le HBox
                            hbox.getChildren().addAll(new Label(evenementSponsor.toString()), btnGeneratePdf); // Ajouter le texte et le bouton
                            setGraphic(hbox); // Définir le HBox comme contenu de la cellule
                        }
                    }
                };
            }
